...
sub run {
    ...
    $replicate_cnt -= $self->dbh->selectrow_array(q{SELECT COUNT(*) FROM file WHERE uuid=?}, {}, $uuid);
    ...
    for my $storage (@storage_nodes) {
        ...
        if ($code == 200) {
            $self->dbh->do(
                q{INSERT INTO file (uuid, bucket_id, size, ext, storage_id) SELECT uuid, bucket_id, size, ext, ? AS storage_id FROM file WHERE uuid=?},
                {},
                $storage->{id},
                $uuid
              )
